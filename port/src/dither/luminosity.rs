...
/// Mixed wide kernel dithering with standard (left-to-right) scanning for grayscale.
/// Randomly selects between FS/32 and Residual/32 per pixel.
#[inline]
fn dither_mixed_wide_standard_gray(
    ctx: &GrayDitherContext,
    gray_channel: &[f32],
    err_buf: &mut [Vec<f32>],
    out: &mut [u8],
...
/// Mixed half kernel dithering with standard (left-to-right) scanning for grayscale.
/// Randomly selects between forward-ish and downward-ish halves of FS per pixel.
fn dither_mixed_half_standard_gray(
    ctx: &GrayDitherContext,
    gray_channel: &[f32],
    err_buf: &mut [Vec<f32>],
    out: &mut [u8],
...
// ============================================================================
// Mixed H2 (second-order kernel) grayscale dithering
// ============================================================================

/// Mixed H2 kernel dithering for colorspace-aware grayscale, LTR only.
#[inline]
fn dither_mixed_h2_standard_gray(
    ctx: &GrayDitherContext,
    gray_channel: &[f32],
    err_buf: &mut [Vec<f32>],
    out: &mut [u8],
...
pub fn colorspace_aware_dither_gray_with_options(
    ...
    match mode {
        ...
        DitherMode::MixedWideStandard => {
            dither_mixed_wide_standard_gray(
                &ctx, gray_channel, &mut err_buf, &mut out,
                width, height, reach, hashed_seed, progress,
            );
        }
        DitherMode::MixedHalfStandard => {
            dither_mixed_half_standard_gray(
                &ctx, gray_channel, &mut err_buf, &mut out,
                width, height, reach, hashed_seed, progress,
            );
        }
        ...
        // (threshold/weight modulation is for per-channel quantization)
