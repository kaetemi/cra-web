...
use super::common::{
    apply_mixed_half_kernel_rgba, apply_mixed_kernel_rgba, apply_mixed_wide_kernel_rgba,
...
use super::kernels::{apply_h2_kernel_rgba, H2_REACH, H2_SEED};
...
/// Mixed half kernel dithering with standard scanning for paletted output.
fn dither_mixed_half_standard_paletted(
    ctx: &DitherContextPaletted,
    r_channel: &[f32],
    g_channel: &[f32],
    b_channel: &[f32],
    ...
    for y in 0..process_height {
        for bx in bx_start..bx_start + process_width {
            ...
            let pixel_hash = lowbias32((px as u32) ^ ((y as u32) << 16) ^ hashed_seed);
            apply_mixed_half_kernel_rgba(err_r, err_g, err_b, err_a, bx, y, err_r_val, err_g_val, err_b_val, err_a_val, pixel_hash);
        }
        if y >= reach {
            if let Some(ref mut cb) = progress {
                cb((y - reach + 1) as f32 / height as f32);
...
/// Mixed half kernel dithering with standard scanning for extended paletted output.
fn dither_mixed_half_standard_paletted_extended(
    ctx: &ExtendedDitherContext,
    r_channel: &[f32],
    g_channel: &[f32],
    b_channel: &[f32],
...
pub fn paletted_dither_rgba_with_mode(
    ...
    match mode {
        ...
        DitherMode::MixedHalfStandard => {
            dither_mixed_half_standard_paletted(
                &ctx, r_channel, g_channel, b_channel, a_channel,
                &mut err_r, &mut err_g, &mut err_b, &mut err_a,
                &mut r_out, &mut g_out, &mut b_out, &mut a_out,
                width, height, reach, hashed_seed, progress,
...
pub fn paletted_dither_rgba_gamut_mapped(
    ...
    match mode {
        ...
        DitherMode::MixedHalfStandard => {
            dither_mixed_half_standard_paletted_extended(
                &ctx, r_channel, g_channel, b_channel, a_channel,
                &mut err_r, &mut err_g, &mut err_b, &mut err_a,
                &mut r_out, &mut g_out, &mut b_out, &mut a_out,
                width, height, reach, hashed_seed, progress,
...
fn paletted_dither_to_indices_impl(
    ...
    let _hashed_seed = lowbias32(seed);
