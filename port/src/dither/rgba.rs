...
/// Mixed wide kernel dithering with standard (left-to-right) scanning for RGBA.
/// Randomly selects between FS/32 and Residual/32 per channel per pixel.
#[inline]
fn dither_mixed_wide_standard_rgba(
    ctx: &DitherContextRgba,
    r_channel: &[f32],
    g_channel: &[f32],
    b_channel: &[f32],
...
/// Mixed half kernel dithering with standard (left-to-right) scanning for RGBA.
/// Randomly selects between forward-ish and downward-ish halves of FS per channel per pixel.
fn dither_mixed_half_standard_rgba(
    ctx: &DitherContextRgba,
    r_channel: &[f32],
    g_channel: &[f32],
    b_channel: &[f32],
...
pub fn colorspace_aware_dither_rgba_with_options(
    ...
    match mode {
        ...
        DitherMode::MixedWideStandard => {
            dither_mixed_wide_standard_rgba(
                &ctx, r_channel, g_channel, b_channel,
                &mut err_r, &mut err_g, &mut err_b,
                &mut r_out, &mut g_out, &mut b_out,
                width, height, reach, hashed_seed, progress,
        ...
        DitherMode::MixedHalfStandard => {
            dither_mixed_half_standard_rgba(
                &ctx, r_channel, g_channel, b_channel,
                &mut err_r, &mut err_g, &mut err_b,
                &mut r_out, &mut g_out, &mut b_out,
                width, height, reach, hashed_seed, progress,
