...
// ============================================================================
// Alpha-aware RGB dithering context and pixel processing for f16
// ============================================================================

/// Context for alpha-aware f16 pixel processing
struct Fp16DitherContext<'a> {
    space: PerceptualSpace,
    working_space: Fp16WorkingSpace,
    /// Pre-dithered alpha channel (f16 values)
    alpha_dithered: &'a [f16],
}
...
#[inline]
fn process_pixel_f16(
    ctx: &Fp16DitherContext,
    r_channel: &[f32],
    g_channel: &[f32],
    b_channel: &[f32],
    err_r: &[Vec<f32>],
    ...
    let alpha_f16 = ctx.alpha_dithered[idx];
    ...
    let (lin_r_orig, lin_g_orig, lin_b_orig) = match ctx.working_space {
        Fp16WorkingSpace::Linear => (r_channel[idx], g_channel[idx], b_channel[idx]),
    ...
    for &r_cand in &r_candidates {
        for &g_cand in &g_candidates {
            for &b_cand in &b_candidates {
                ...
                let r_lin = f16_to_linear(r_cand, ctx.working_space);
                let g_lin = f16_to_linear(g_cand, ctx.working_space);
                let b_lin = f16_to_linear(b_cand, ctx.working_space);
    ...
    let best_lin_r = f16_to_linear(best_r, ctx.working_space);
    let best_lin_g = f16_to_linear(best_g, ctx.working_space);
    let best_lin_b = f16_to_linear(best_b, ctx.working_space);
    ...
    for y in 0..height {
        for x in 0..width {
            ...
            let (best_r, best_g, best_b, err_r_val, err_g_val, err_b_val) =
                process_pixel_f16(ctx, r_channel, g_channel, b_channel, err_r, err_g, err_b, idx, bx, y);
