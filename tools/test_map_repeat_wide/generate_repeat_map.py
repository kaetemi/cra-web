#!/usr/bin/env python3
"""
Recursive ranked dither map with wrapping error diffusion (wide kernel variant).

Uses FS/32 + Residual/32 kernel pair instead of FS/JJN.
- FS/32 (FS×2):        *  14  /  6  10  2
- Residual/32:          *   0   5  /  3   2   2   4   3  /  1   3   5   3   1

Same reach as JJN (2 cols, 3 rows), same buffer layout.
...
@njit(cache=True)
def apply_error_wrap(buf, x, y, height, width, err, use_wide):
    """Apply FS/32 or Residual/32 error with full toroidal wrapping (LTR only)."""
    if use_wide:
        # Residual kernel /32 (LTR)
        #     *   0   5
        # 3   2   2   4   3
        # 1   3   5   3   1
        buf[y, (x + 2) % width] += err * (5.0 / 32.0)
        y1 = (y + 1) % height
        buf[y1, (x - 2) % width] += err * (3.0 / 32.0)
        buf[y1, (x - 1) % width] += err * (2.0 / 32.0)
        buf[y1, x] += err * (2.0 / 32.0)
        buf[y1, (x + 1) % width] += err * (4.0 / 32.0)
        buf[y1, (x + 2) % width] += err * (3.0 / 32.0)
        y2 = (y + 2) % height
        buf[y2, (x - 2) % width] += err * (1.0 / 32.0)
        buf[y2, (x - 1) % width] += err * (3.0 / 32.0)
        buf[y2, x] += err * (5.0 / 32.0)
        buf[y2, (x + 1) % width] += err * (3.0 / 32.0)
        buf[y2, (x + 2) % width] += err * (1.0 / 32.0)
    else:
        # FS /32 (FS×2) (LTR)
        #    *  14
        # 6  10  2
        buf[y, (x + 1) % width] += err * (14.0 / 32.0)
        y1 = (y + 1) % height
        buf[y1, (x - 1) % width] += err * (6.0 / 32.0)
        buf[y1, x] += err * (10.0 / 32.0)
        buf[y1, (x + 1) % width] += err * (2.0 / 32.0)
    ...
    for j in range(height):
        ...
        for p in range(row_passes):
            for i in range(width):
                ...
                use_wide = (coord_hash & 1) == 1

                apply_error_wrap(buf, x, y, height, width, err, use_wide)
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate tileable recursive ranked dither map (wide kernel, wrapping error diffusion)'
    )
    ...
    print(f"Wrapping error diffusion (wide kernel): {size}x{size}, {row_passes} passes/row")
    if use_tpdf:
