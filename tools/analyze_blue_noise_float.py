#!/usr/bin/env python3
"""
Analyze blue noise RNG floating-point output.

Generates:
  1. Spectral analysis of nextf() output at 16-bit depth
  2. Distribution histogram binned at 16-bit resolution (65536 bins)
  3. 30-second float32 WAV files centered around 0

Usage:
    python tools/analyze_blue_noise_float.py
    python tools/analyze_blue_noise_float.py --count 10485760 --seed 42
    python tools/analyze_blue_noise_float.py --no-wav
"""
...
def generate_spectrum(count, seed, output_dir):
    ...
    fig.suptitle(
        f'Blue Noise RNG nextf() Spectral Analysis ({count:,} samples, seed {seed})',
        fontsize=14
    )
...
def generate_distribution(count, bit_depth, seed, output_dir):
    ...
    fig.suptitle(
        f'Blue Noise RNG nextf() Distribution \u2014 65536 bins '
    ...
    plt.tight_layout()
    path = output_dir / f'distribution_float_16bit_bd{bit_depth}.png'
    plt.savefig(path, dpi=150)
    plt.close()
...
def generate_wav(seed, output_dir):
    ...
    for sample_rate in [8000, 48000]:
        ...
        path = output_dir / f'blue_noise_rng_float_{label}.wav'
        write_wav_float32(path, sample_rate, centered)
        size_kb = path.stat().st_size / 1024
        print(f"    Saved: {path} ({size_kb:.0f} KB)")

        # Reference white noise for comparison
        rng_ref = np.random.default_rng(seed)
        white = (rng_ref.random(n_samples) - 0.5).astype(np.float32)

        path_w = output_dir / f'ref_white_noise_float_{label}.wav'
        write_wav_float32(path_w, sample_rate, white)
        print(f"    Saved: {path_w}")

        # Reference blue noise (filtered white noise, +6dB/oct)
        fft_w = np.fft.rfft(white)
        freqs = np.fft.rfftfreq(n_samples, d=1.0 / sample_rate)
        freqs[0] = 1.0  # avoid div by zero
        fft_w *= np.sqrt(freqs / freqs[len(freqs) // 2])
        filtered = np.fft.irfft(fft_w, n=n_samples)
        # Normalize to [-0.5, 0.5]
        filtered = filtered / (np.abs(filtered).max() + 1e-30) * 0.5
        filtered = filtered.astype(np.float32)

        path_ref = output_dir / f'ref_blue_noise_float_{label}.wav'
        write_wav_float32(path_ref, sample_rate, filtered)
        print(f"    Saved: {path_ref}")
