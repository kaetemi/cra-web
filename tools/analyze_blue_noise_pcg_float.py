#!/usr/bin/env python3
"""
Analyze blue noise RNG (PCG64-DXSM variant) floating-point output.

Same tests as analyze_blue_noise_float.py but using the PCG64-DXSM variant
for comparison against the lowbias32 and mt19937 versions.

Usage:
    python tools/analyze_blue_noise_pcg_float.py
    python tools/analyze_blue_noise_pcg_float.py --count 10485760 --seed 42
    python tools/analyze_blue_noise_pcg_float.py --no-wav
"""
...
_BIN_PATH = Path(__file__).parent / 'blue_noise_rng_pcg_float_test'
...
def generate_spectrum(count, seed, output_dir):
    ...
    fig.suptitle(
        f'Blue Noise RNG (PCG64-DXSM) nextf() Spectral Analysis ({count:,} samples, seed {seed})',
        fontsize=14
    )
    ...
    # White noise references from all three RNG backends
    ...
    white_pcg = get_white_samples(_BIN_PATH, count, seed)
    fft_wpcg = np.fft.rfft(white_pcg - white_pcg.mean())
    power_wpcg = np.abs(fft_wpcg) ** 2 / count
    sf_wpcg, sp_wpcg = smooth_spectrum(freqs[mask], power_wpcg[mask])
    ...
    # Smoothed blue noise
    ax.loglog(sf, sp, color='#27ae60', linewidth=2, label=f'nextf() bd={bd} (PCG64-DXSM)')
    ...
    # White noise: PCG64-DXSM
    ax.loglog(sf_wpcg, sp_wpcg, color='purple', linewidth=1, linestyle='--', alpha=0.6, label='White (PCG64-DXSM)')
    # Ideal +6 dB/oct
    ...
    plt.tight_layout()
    path = output_dir / 'spectrum_blue_noise_pcg_float.png'
...
def generate_distribution(count, bit_depth, seed, output_dir):
    ...
    fig.suptitle(
        f'Blue Noise RNG (PCG64-DXSM) nextf() Distribution \u2014 65536 bins '
    ...
    plt.tight_layout()
    path = output_dir / f'distribution_pcg_float_16bit_bd{bit_depth}.png'
    plt.savefig(path, dpi=150)
    plt.close()
...
def generate_wav(seed, output_dir):
    ...
    for sample_rate in [8000, 48000]:
        ...
        path = output_dir / f'blue_noise_rng_pcg_float_{label}.wav'
...
def main():
    parser = argparse.ArgumentParser(
        description='Analyze blue noise RNG (PCG64-DXSM) floating-point output'
    )
    ...
    print("=== Spectral analysis (nextf PCG64-DXSM, bit_depth=16) ===")
    generate_spectrum(args.count, args.seed, output_dir)

    print(f"\n=== Distribution histogram (nextf PCG64-DXSM, bit_depth={args.dist_bits}) ===")
    generate_distribution(args.dist_count, args.dist_bits, args.seed, output_dir)

    if not args.no_wav:
        print(f"\n=== Float32 WAV files (30s, centered, PCG64-DXSM) ===")
